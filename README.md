...
浏览器本地完成解码、裁切、像素交换和导出。只有用户主动点击生成或重试建议，且存在待补全的色纸或形状时，模型才接收最长边 768 px 的预览；服务端再次解码、限制尺寸并清除元数据。原始上传文件不保存到服务器。

`base photos/` 和 `reference/` 为本地素材，不进入版本控制或生产静态资源；测试样片通过受限的文件名接口从 `SAMPLE_DIR` 提供。没有该目录时不显示样片，上传功能照常可用。服务不会输出密钥、原始模型响应或图片内容到日志。

README 的界面封面和三张作品案例收录于 `docs/images/`，是选定用于公开展示的图片；应用启动和测试不依赖这些图片。

## GitHub 与源码发布

仓库包含源码、依赖锁文件、环境变量模板、测试和 GitHub Actions。CI 使用合成测试图和模拟模型，无真实密钥、无私人照片也能运行；可选的本地样片测试会自动跳过。
...
源码包输出到 `.release/punch-studio-版本号-source.zip`，内容来自已检查的 Git 暂存区，不包含 `.git` 历史或被忽略的本地文件。修改后需重新 `git add .` 再打包。贡献流程见 [CONTRIBUTING.md](CONTRIBUTING.md)。

当前版本是单用户本地工作台：不包含账号、跨设备草稿或项目持久化，刷新会清空编辑状态。若以后提供公共服务，应在外层增加身份认证、用户隔离和统一的用量控制；当前内存缓存和限流面向单进程运行。
